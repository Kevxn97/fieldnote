...
function buildCommandHubLines(paint: TerminalPainter): string[] {
  return [
    ...
    `  ${paint.cyan("/heal")}     ${paint.gray("h")}   run a wiki health audit and optionally apply additive fixes`,
    `  ${paint.cyan("/evolve")}   ${paint.gray("e")}   run an experimental deeper maintenance pass for entities and concepts`,
    "",
    paint.dim("BROWSE"),
